...
def predict_by_input():
    try:
        ...
        # Combine categorical data as trained (using dynamic form values)
        combined_cats = f"{gender} {geography} {request.form['native_place']} {marital_status} {request.form['account_type']} {request.form['loan_status']} {request.form['branch_code']} {request.form['review_sentiment']}"
        
        # Apply TF-IDF to the combined categorical features
        ...
        numeric_data = [[
            ...
            monthly_income, credit_score, emi_amount
        ]]
        numeric_df = pd.DataFrame(scaler.transform(numeric_data), columns=[ 
            'Age', 'Tenure', 'Balance', 'NumOfProducts', 'EstimatedSalary',
            'Monthly_Income', 'CreditScore', 'EMI_Amount'
        ])

        # Combine all inputs (numeric and TF-IDF)
